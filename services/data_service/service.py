    ...
    async def warmup_data(self) -> Dict[str, int]:
        ...
        for pair in self.pairs:
            for tf in self.timeframes:
                ...
                start = now - min_hist
                tasks.append((pair, tf, start, now))

        # Sequential to respect rate limits, but could be parallelized per-exchange
        results: Dict[str, int] = {}
        for pair, tf, start, end in tasks:
            try:
                count = await self.fetch_and_store(pair, tf, start, end)

...
class LearningService:
    ...
    # Default stability threshold: max allowed score drop from avg (as fraction)
    DEFAULT_STABILITY_THRESHOLD = 0.30
    ...
    def __init__(self, config: Optional[LearningConfig] = None):
        ...
        self._trade_count = 0
        # Stability threshold from config, or default
        self.stability_threshold = getattr(
            self.config, "stability_threshold", self.DEFAULT_STABILITY_THRESHOLD
        )
    ...
    def _compute_window_metrics(self, trades: List[dict]) -> dict:
        ...
        # Sharpe (simplified) — safe division
        returns = [t["pnl"] for t in trades]
        n = len(returns)
        if n == 0:
            sharpe = 0.0
        elif n == 1:
            sharpe = float(returns[0]) if returns[0] != 0 else 0.0
        else:
            ...
            if std_ret > 1e-10:
                ...
                sharpe = 0.0  # zero variance → zero Sharpe (no edge detectable)
        ...
        dd = float(np.max(peak - cumulative)) if len(cumulative) > 0 else 0.0
    ...
    def multi_criteria_score(self, metrics: dict) -> float:
        ...
        score = (
            w.get("sharpe", 0.35) * sharpe_norm +
            w.get("profit_factor", 0.25) * pf_norm +
            w.get("drawdown", 0.20) * dd_norm +
            w.get("stability", 0.20) * stability_norm
        )
        return round(score, 4)

    def is_stable(self, min_windows: Optional[int] = None) -> bool:
        """
        Check if Walk Forward results show stability.
        Requires `min_windows` consecutive windows without significant degradation.

        Args:
            min_windows: Override default from config. Uses self.config.min_windows if None.

        Returns:
            True if all recent windows are within stability_threshold of the average.
        """
        ...
        for s in scores:
            if (avg_score - s) / avg_score > threshold:
                return False

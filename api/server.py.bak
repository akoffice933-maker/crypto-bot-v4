...
def create_app(bot=None) -> FastAPI:
    ...
    async def close_position(pair: str, request: Request, _auth=Depends(require_auth)):
        ...
        b.portfolio_engine.close_position(pair, exit_price, 0)
        ws_mgr.broadcast("position.closed", {"pair": pair})
        capture_log("INFO", f"Position closed via UI: {pair}", service="api")
        return {"status": "ok", "pair": pair, "closed": True}
    ...
    async def move_stop_loss(pair: str, request: Request, _auth=Depends(require_auth)):
        ...
        ok = b.portfolio_engine.update_stop_loss(pair, new_sl)
        if not ok:
            raise HTTPException(status_code=404, detail="Position not found")
    ...
    async def bot_status(request: Request):
        ...
        return {"running": b._running if b else False,
                ...
                "version": b.config.version if b else "5.0.0",
                "uptime_seconds": 0,

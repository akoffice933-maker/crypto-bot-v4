...
from sqlalchemy import (
    ...
    PrimaryKeyConstraint, create_engine, event, Index, insert,
)
...
class DatabaseManager:
    ...
    def insert_market_data_batch(self, records: list[dict], session: Optional[Session] = None):
        """
        Bulk-insert market data records with upsert (ON CONFLICT DO NOTHING).

        Uses a single SQL statement instead of N individual merges.
        Falls back to per-row merge for SQLite < 3.24 (rare).
        """
        close_session = False
        if session is None:
        ...
        try:
            ...
            if self._is_sqlite:
                # SQLite: use INSERT OR IGNORE via sqlite dialect
                stmt = sqlite_insert(MarketData).values(records)
                stmt = stmt.on_conflict_do_nothing(index_elements=["timestamp", "pair", "timeframe"])
                session.execute(stmt)
            else:
                # PostgreSQL: native ON CONFLICT
